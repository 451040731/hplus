		...
		methods:{
			...
			*/
			...
			createCanvasImage(parent) {
				...
				ctx.save();
				this.roundRect(ctx,50,40,(this.phoneW-100), (345)*scaleH,10,'#f7f7f7','#f7f7f7');//绘制海报圆角背景 上半截灰色的
				ctx.restore();
				//将网络图片转成本地路径 商品图片
			    wx.getImageInfo({
			       src: url,
			...
			      }  ,
				...
				ctx.fillText(zpPrice, 75, 376*scaleH,zpPriceW);
			
				//绘制价格 end
				wx.getImageInfo({
			       src: code,
			       success(res) {
					ctx.drawImage(res.path,(_this.phoneW-129)/2,400*scaleH, 129, 129)
					ctx.draw(true)
			      },
					fail(){
						_this.canvasFlag=true;
				...
				ctx.fillText('长按/扫描识别小程序', (_this.phoneW-140)/2, 560*scaleH,140);
				// 长按/扫描识别查看商品end
				//绘制保存按钮
				ctx.save(); 
			...
			saveCanvasImage(){
				...
				success(res) {
					...
					fail(msg) {
						console.log(msg);
						let title = '未授权相册权限';
						if("saveImageToPhotosAlbum:fail cancel" == msg.errMsg){
							title = '取消保存';
						}
