	...
	<view>
		...
		<mescroll-uni top="0" @down="downCallback" :up="upOption" @up="upCallback" @emptyclick="emptyClick" @init="mescrollInit">
			...
			<view class="promotion-wrap">
				<view class="promotion-item" v-for="(item, index) in pdList" :key="index">
					<view class="name b-b">
						<text>商品名：{{item.goodsName}}</text>
						<text class="time">{{item.createTime}}</text>
					</view>
					<view class="con">
						<view class="mix">提货金额：￥{{item.money}}</view>
						<view class="mix">剩余产品数量：{{item.remainTakeNum}}</view>
						<view class="mix">该次提货数量：{{item.takeNum}}</view>
					</view>
				</view>
			</view>
		</mescroll-uni>
	...
	export default {
		...
		methods: {
			...
			getListDataFromNet(page,limit,successCallback,errorCallback) {
				this.$api.takegoods(this.$qs.stringify({
					page: page,
					limit: limit
				})).then(res => {
					if(res.data.ok){
